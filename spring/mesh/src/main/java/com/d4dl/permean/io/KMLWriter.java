 ...
 public void outputKML(MeshCell[] cells, boolean initiator18Only, int limit) throws IOException {
    ...
    try {
      ...
      for (int i = 0; i < length; i++) {
        int styleIndex = Sphere.initiatorKey18Percent.equals(cells[i].getInitiator()) ? 0 : 2;
        if (!initiator18Only || Sphere.initiatorKey18Percent.equals(cells[i].getInitiator()))
  ...
  private String getPolygon(MeshCell cell) {
    return
        "      <Polygon>\n" +
            "      <outerBoundaryIs>\n" +
            "      <LinearRing>\n" +
