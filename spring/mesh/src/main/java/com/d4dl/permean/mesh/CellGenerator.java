...
public class CellGenerator {
   ...
   */
  public MeshCell populateCell(int cellIndex, String initiator, Map<String, MeshVertex> vertexCache, AtomicInteger currentVertexIndex) {
    UUID id = UUID.randomUUID();
    return new DefaultCell(initiator, id, populateVertices(cellIndex, vertexCache, currentVertexIndex), 0, (float)barycenters[cellIndex].getLat(), (float)barycenters[cellIndex].getLng());
  }

  public MeshVertex[] populateVertices(int cellIndex, Map<String, MeshVertex> vertexCache, AtomicInteger currentVertexIndex) {

    boolean isPentagon = isPentagon(cellIndex);
    MeshVertex[] vertices = new DefaultVertex[isPentagon ? 5 : 6];
    ...
    for (int i = 0; i < adjacentCellIndices.length; i++) {
      ...
      String stableKey = createStableKey(cellIndex, firstAdjacentIndex, secondAdjacentIndex);
      MeshVertex sharedVertex = vertexCache != null ? vertexCache.get(stableKey) : null;
      if (sharedVertex == null) {
        ...
        if (currentVertexIndex != null) {
          ...
          sharedVertex = new DefaultVertex(stableKey, vertexIndex, (float) centroid.getLat(), (float) centroid.getLng());
        } else {
          sharedVertex = new DefaultVertex(stableKey, 0, (float) centroid.getLat(), (float) centroid.getLng());
        }
        if(vertexCache != null) {
          vertexCache.put(stableKey, sharedVertex);
        }
      } else if (vertexCache != null) {
        sharedVertex.access();
        if (sharedVertex.getAccessCount() == 2) {
          vertexCache.remove(sharedVertex.getStableKey());
        }
      }
  ...
  private String createStableKey(int i0, int i1, int i2) {
    StringBuffer buffer = new StringBuffer();
    int[] sharedVertexKey = new int[]{i0, i1, i2};
    Arrays.sort(sharedVertexKey);
    buffer.append(sharedVertexKey[0]);
    buffer.append(sharedVertexKey[1]);
    buffer.append(sharedVertexKey[2]);
   ...
   * @return
   */
  public MeshCell[] getPentagons(int cellCount, Map<String, MeshVertex> vertexMap, AtomicInteger currentVertexIndex) {
    MeshCell[] pentagons = new MeshCell[EUCLID_DETERMINED_PENTAGONS_IN_A_TRUNCATED_ICOSOHEDRON];
    AtomicInteger cellCounter = new AtomicInteger();
    IntStream parallel = IntStream.range(0, cellCount).parallel();
    parallel.forEach(f -> {
      if(isPentagon(f)) {
